...
f = open('data3.csv','w')
wr= csv.writer(f)
robot_class = '/home/d109master/robot/my_robot1'
img_path_list = []
my_robot_img=[]
aother_img=[]
...
for i in range(len(my_robot_img)):
    # print(i)
    red_hsv = cv2.cvtColor(my_robot_img[i], cv2.COLOR_BGR2HSV)
...
###################################################
aother_class = '/home/d109master/robot/another1'
img_path_list = []
...
for i in range(len(aother_img)):
    # print(i)
    red_hsv = cv2.cvtColor(aother_img[i], cv2.COLOR_BGR2HSV)
    # img = cv2.imread('box.jpg')
    channels = cv2.split(red_hsv)
    colors = ('b', 'g', 'r')
    for (ch, color) in zip (channels, colors):
