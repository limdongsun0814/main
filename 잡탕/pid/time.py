...
f = open('step per sec.csv','r')
print('step per sec.csv.csv')
rdr = csv.reader(f)
x,y=[],[]
a=1#4
b=2#2
...
for line in rdr:
    if sw == 1:
        ...
        times_arr.append(float(line[5])-float(line[4]))
        times_arr_two.append(float(line[7])-float(line[6]))
        tar_d.append(0.00447755932808)
        tar_a.append(0.00563910126686)
...
if sw ==1:
    ...
    plt.plot(times_arr, 'r',marker='o',label='Tracking Robot identification')
    plt.plot(times_arr_two, marker='o',label='Tracking Robot position calculation')
    plt.plot(tar_d,'y--')#, 'ko')
    plt.plot(tar_a,'b--')#, 'ko')
    plt.ylim(0.00,0.01)
    plt.legend(loc=(0.6, 1.0))
    print(sum(times_arr)/len(times_arr))
    print(sum(times_arr_two)/len(times_arr_two))
# plt.ylim(40,110)
# if sw == 1:
#     plt.figure(1)
