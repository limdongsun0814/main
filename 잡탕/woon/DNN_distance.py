...
normarl=1.0
x_noraml_one = math.pi
x_noraml_two = 2*math.pi
...
model.add(layers.Dense(3, input_shape=(1,),activation="linear"))
...
optimizer=tf.keras.optimizers.Adam(
    lr=10 ** -5#, beta_1=0.95, beta_2=0.999#, epsilon=None, decay=0.0, amsgrad=False
)
model.compile(optimizer=optimizer,
              loss='mse',#)#,#tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),    /mse
               metrics=['mae'])
hist=model.fit((x_train_data.T+x_noraml_one)/x_noraml_two,y_train_data.T*normarl, epochs=10000,batch_size=100,
                    validation_data = ((x_test_data.T+x_noraml_one)/x_noraml_two, y_test_data.T*normarl))
# print(model.summary())
...
y=model.predict((x_test_data.T+x_noraml_one)/x_noraml_two)
...
aa.plot(y_test_data,(y.T/normarl-y_test_data).T,'r*')
# print(xy_data.shape)
