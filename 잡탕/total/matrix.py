...
# datasets = np.loadtxt('data3.csv', delimiter=',',skiprows=1)
# xy_data = datasets
...
# # print("[y]", y.dtype)
# y=reconstructed_model.predict_classes(x_data)
# cf = confusion_matrix(y_data, y)
cf= np.array([[1275,1],[0,1182]])#test
# cf= np.array([[2949,0],[0,2786]])#train
# for i in range(len(y)):
#     print(y_train_data.T[i],y[i])
# data_csv=np.abs(y_data-y)
# for i in range(len(data_csv)):
#     data=[]
#     data.append(data_csv[i])
#     print(data_csv[i])
#     # data.append(fil[i])
#     wr.writerow(data)
print(cf)
...
ax.set_title('test data set Confusion Matrix')
