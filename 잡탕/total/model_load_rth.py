...
reconstructed_model = load_model("data_linear_angle_r.h5",compile=False)
...
x_pre = r_pre * np.sin(np.radians(th_pre-90))
y_pre = r_pre * np.cos(np.radians(th_pre-90))



th_rel = y_data.T[1]+70
r_rel = y_data.T[0]+35

x_rel = r_rel * np.sin(np.radians(th_rel-90))
y_rel = r_rel * np.cos(np.radians(th_rel-90))



plt.scatter(x_rel,y_rel, s=2**2, c='r')
plt.scatter(x_pre,y_pre, s=2**2, c='b')
#
plt.plot([x_rel,x_pre],[y_rel,y_pre],color="green")#,linewidth='10')
